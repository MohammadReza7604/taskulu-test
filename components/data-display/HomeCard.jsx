import { Card } from "antd";
import React from "react";
import { Chart as ChartJS, ArcElement, Tooltip, Legend } from "chart.js";
import { Doughnut } from "react-chartjs-2";
import { StarFilled } from "@ant-design/icons";
import Image from "next/image";
import Title from "antd/lib/typography/Title";
import classes from "./styles/HomeCard.module.css";
import Text from "antd/lib/typography/Text";
import { CardAvatar } from "./CardAvatar";

ChartJS.register(ArcElement, Tooltip, Legend);
export const HomeCard = (props) => {
  const data = {
    datasets: [
      {
        labels: "# of Votes",
        data: [30, 5, 14],
        backgroundColor: [
          "rgba(255, 138, 128, 1)",
          "rgba(255, 179, 0, 1)",
          "rgba(124, 179, 66, 1)",
        ],
        borderColor: [
          "rgba(255, 138, 128, 1)",
          "rgba(255, 179, 0, 1)",
          "rgba(124, 179, 66, 1)",
        ],
        borderWidth: 1,
      },
    ],
  };
  return (
    <Card
      className={classes.card}
      onClick={props.onClick}
      extra={
        <div className={classes.chart_card}>
          <div className={classes.title}>
            <StarFilled />
            <Image width={20} height={20} src={"/images/office.png"} />
            <Title level={5}>iranfiles</Title>
          </div>
          <div className={classes.chart}>
            <Doughnut data={data} />
          </div>
          <div className={classes.items}>
            <Text>Done</Text>
            <Text>Doing</Text>
            <Text>Todo</Text>
          </div>
          <div className={classes.items_value}>
            <Text>40</Text>
            <Text>4</Text>
            <Text>143</Text>
          </div>
        </div>
      }
    >
      <div className={classes.item_avatar}>
        <CardAvatar
          className={classes.avatar}
          src="https://dictionary.cambridge.org/images/thumb/flower_noun_002_14403.jpg?version=5.0.243"
        />
        <CardAvatar src="data:image/png;base64," />
        <CardAvatar src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcStUA8ai6tKwZebp5iAZuNJo7_JFUblRVHzag&usqp=CAU" />
        <CardAvatar src="https://www.sciencenewsforstudents.org/wp-content/uploads/2019/11/Flower-772x579.jpg" />
        <Text>+3 نفر دیگر</Text>
      </div>
    </Card>
  );
};
